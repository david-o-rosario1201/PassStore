...
@Composable
fun ConfirmSecurityCode(
    title: String,
    context: Context,
    onEvent: (SettingUiEvent) -> Unit
){
    Dialog(onDismissRequest = { onEvent(SettingUiEvent.DismissAllModals)}) {
        Card(
            ...
            Column(
                ...
                //Pregunta
                Text(
                    text = context.getString(R.string.confirm_modal_question),
                    color = MaterialTheme.colorScheme.onSurface,
                    fontSize = 16.sp,
                    modifier = Modifier.align(Alignment.CenterHorizontally)
                )
                ...
                Row {
                    ...
                    Button(
                        onClick = { onEvent(SettingUiEvent.Save)},
                        colors = ButtonDefaults.buttonColors(
